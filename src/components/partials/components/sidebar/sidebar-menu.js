...
const SidebarMenu = memo((props) => {
  ...
  return (
    <Fragment>
      {(props.isTag === 'true' && !props.isAction) &&
      ...
      {(props.isTag === 'true' && props.isAction === 'true') &&
        <li className={`${location.pathname === `${props.pathname}` ? 'active' : ''} nav-item `}>
          <div className={`${location.pathname === `${props.pathname}` ? 'active' : ''} nav-link `} aria-current="page" onClick={props.handleLogout}>
              <OverlayTrigger placement="right" overlay={<Tooltip>{props.title}</Tooltip>}>
                {props.children}
              </OverlayTrigger>
              <span className="item-name">{props.title}</span>
