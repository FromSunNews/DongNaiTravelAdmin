...
// Cách sử dụng: Với tất cả các link hoặc button mà có hành động call api thì thêm class "promise-send-btn" cho nó là xong.

const updateSendingApiStatus = (sending = true) => {
  const submits = document.querySelectorAll('.promise-send-btn')
  for (let i = 0; i < submits.length; i++) {
    if (sending) submits[i].classList.add('pending-send-btn')
    else submits[i].classList.remove('pending-send-btn')
  }
}
